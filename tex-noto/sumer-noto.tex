...
\begin{tabular}[!h]{l | l l l l l}
  Hamtu & Marû \\
{\fcm 𒈗𒂊𒀀𒈾𒈬𒌦𒀝} & {\fcm 𒈬𒊏𒀊𒋧𒂊}\\
lugal e a na mu un ak & mu ra ab shum2 e \\
What did the king do? & He should give it to you.\\
\end{tabular}\verb||\\

\verb||\\
The reduplication class forms the
marû by reduplicating the
hamtu stem. The example below
shows the  pronominal conjugation,
which is equivalent to the English when-clause,
with the reduplicating verb {\fcn 𒍣} (zi).
The pronominal conjugation consists of a verb
nominalized with -/a/ and a following
possessive pronoun referring
to the subject. 
...
\begin{tabular}[!h]{l | l l l l l l l l}
  Hamtu & Marû\\
\fcm 𒍣𒂵𒈬𒉈 &  \fcm 𒍣𒍣𒁕𒈬𒉈 \\
  zig3 ga ĝu10 ne & zi zi da ĝu10 ne\\
  as I rose  & when I rise\\
\end{tabular}


\section{Regular verbs}
The class of regular verbs comprises
70\% Sumerian verbs. In this class,
the only way to distinguish the
imperfective aspect from the perfective
is that the conjugation of the hamtu
form is often realized with pronominal
prefixes, whereas the marû conjugation is
accomplished with pronominal suffixes.
\index{ak {\fcn 𒀝}  ! to do}
\index{aĝ2 {\fcn 𒉘}  ! to measure}
\index{sar {\fcn 𒊬}  ! to write}
\index{ba {\fcn 𒁀}  ! to divide}
\index{bar {\fcn 𒁇}  ! to open}
\index{bala {\fcn 𒁄}  ! to cross}
\index{igi//bar {\fcn 𒅆} //{\fcn 𒁇}  ! to inspect}
\index{dab5 {\fcn 𒆪}  ! to seize}
\index{de2 {\fcn 𒌤}  ! to pour}
\index{dul {\fcn 𒌋𒌆}  ! to cover}
\index{gu7 {\fcn 𒅥}  ! to eat}

\verb||\\
\begin{tabular}[!h]{l l l | l l l}
  {\fcm 𒋧} & shum2 & to give
  & {\fcm 𒉘}  & aĝ2 & to measure\\
  &&\\
  {\fcm 𒊬} & sar & to write
  & {\fcm 𒀝}  & ak & to do\\
  &&\\
  {\fcm 𒁀}  & ba & to divide
  & {\fcm 𒁀}  // {\fcm 𒀠}  & ba // al & to dig\\
  &&\\
  {\fcm 𒁇}  & bar & to open
  & {\fcm 𒁄} & bala & to cross\\
  &&\\
  {\fcm 𒅆}  // {\fcm 𒁇}  & igi // bar & to inspect
  & {\fcm 𒆪} & dab5 & to seize\\
  &&\\
  {\fcm 𒌤} & de2 & to pour
  & {\fcm 𒁳}  & dib & to pass by\\
  &&\\
  {\fcm 𒆕}  & du3 & to build
  & {\fcm 𒂃}  & du8 & to bake\\
  &&\\
  ...
  &&\\
  {\fcm 𒅍} & il2 & to carry
  & {\fcm 𒋻} & ku5 & to cut off\\
  &&\\
  {\fcm 𒉽} & kur2 & to change
  & {\fcm 𒇲} & la2 & to bind\\
  &&\\
  {\fcm 𒅆𒊒}  & pad3 & to call
  & {\fcm 𒊬}  & sar & to write\\
  ...
  &&\\
\end{tabular}
\index{kur2 {\fcn 𒉽}  ! to change}
\index{la2 {\fcn 𒇲}  ! to bind}
\index{tar {\fcn 𒋻}  ! to cut}

\newpage
Since they are so
important to distinguish the marû
from the hamtu form, below there
is a table of the suffix pronouns
for the two forms.\\

\verb||\\
\begin{tabular}[!h]{l | l |  l | l l l l l}
  &\large\bf Hamtu &\large\bf Marû\\
  \hline
   & \multicolumn{2}{c |}{} \\
  1st person sing. & \multicolumn{2}{c |}{ {\fcm 𒂗}  (en)}\\
  \hline
   & \multicolumn{2}{c |}{} \\
  2nd person sing. & \multicolumn{2}{c |}{ {\fcm 𒂗}  (en)} \\
  \hline
  &&\\
  3rd person sing. & {\fcm 𒂊}  (e) & $\emptyset$\\
  \hline
  & \multicolumn{2}{c |}{}\\
  2nd person pl. &
  \multicolumn{2}{c |}{ {\fcm 𒂗𒉈𒂗}  (en de3 en)}\\
  \hline
  & \multicolumn{2}{c |}{}\\
  2nd person pl. &
  \multicolumn{2}{c |}{ {\fcm 𒂗𒍢𒂗} (en ze2 en)}\\
  \hline
  &&\\
  3rd person pl. & {\fcm 𒂊𒉈}  (e ne)
  & {\fcm 𒌍}  or {\fcm 𒂠}  (esh)\\
\end{tabular}\verb||\\

\paragraph{Hamtu}\verb||\\
{\fcm 𒈬𒊏𒀊𒋧}  \\
mu ra ab shum2\\
He gave it to you.
